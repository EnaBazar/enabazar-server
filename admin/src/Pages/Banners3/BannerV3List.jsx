...
const BannerV3List = () => {
  ...
  const getData = async () => {
    const res = await fetchDataFromApi('/bannerV3');
  ...
  const handleConfirmDelete = async () => {
    if (!deleteId) return;
    await deleteData(`/bannerV3/${deleteId}`);
    context.openAlertBox('success', 'BannerV3 deleted');
    getData();
  ...
  return (
    <>
      <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between py-3 gap-3'>
        <h2 className='text-[20px] font-[600]'>BannerV3 List</h2>
        <Button
          className='btn-blue btn-sm !text-white flex items-center gap-2'
          onClick={() => context.setIsOpenFullScreenPanel({ open: true, model: 'Add BannerV3' })}
        >
