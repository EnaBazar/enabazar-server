...
const AddRams = () => {
  ...
  // 🔥 Open confirm dialog for single delete
  ...
  // 🔥 Open confirm dialog for multiple delete
  ...
  const handleConfirmDelete = async () => {
    if (deleteType === "single" && deleteId) {
      await deleteData(`/product/productRAMS/${deleteId}`);
      context.openAlertBox("success", "Product Deleted");
    } else if (deleteType === "multiple" && selectedItems.length > 0) {
      for (let id of selectedItems) {
        await deleteData(`/product/productRAMS/${id}`);
      }
  ...
  const editItem = (id) => {
    fetchDataFromApi(`/product/productRAMS/${id}`).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId === "") {
      postData(`/product/productRAMS/create`, { name }).then((res) => {
        if (res?.error === false) {
      ...
      editData(`/product/productRAMS/updateProductRams/${editId}`, { name }).then((res) => {
...
<div className="flex flex-col items-start gap-4 w-full">
  ...
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 py-0 mt-3 w-full">
    <h2 className="text-[20px] font-[600]">
      Products Rams <span className="text-[12px] font-[600]">(Material UI Table)</span>
    </h2>
  </div>
  ...
  <Dialog open={openConfirm} onClose={() => setOpenConfirm(false)}>
    ...
    <DialogContent>
      <DialogContentText>
        {deleteType === "multiple"
          ? `আপনি কি নিশ্চিত ${selectedItems.length} টি RAM ডিলিট করতে চান?`
          : "আপনি কি নিশ্চিত এই RAM ডিলিট করতে চান?"}
      </DialogContentText>
    </DialogContent>
    <DialogActions>
