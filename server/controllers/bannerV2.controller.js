import bannerV2model from '../../server/models/bannerV2.model.js';
...
export async function uploadBannerV2Images(request, response) {
 ...
 export async function createBannerV2(request, response) {
  try {
 ...
 export async function getBannersV2(request,response) {
     try{
        const bannersV2 = await bannerV2model.find() ;
      if(!bannersV2) {
   ...
   // get single category
    ...
    export async function getBannerV2(request,response) { 
        try{
            const bannerV2 = await bannerV2model.findById(request.params.id);
            if(!bannerV2){   
                response.status(500).json({  
  ...
  // Remove BannerV1 Image
   ...
   export async function deleteBannerV2(request,response) {
    ...
    const images = bannerV2.images;
     ...
     const deletedbannerV2 = await bannerV2model.findByIdAndDelete(request.params.id);
